...
#### undefined 未定义的，变量声明了，但是未赋值，默认值就是underfined
...
### NaN的说明（表示非数字，指一个无法用数字表示的数字，not a number）
...
#### 三元运算符（三个值参与运算，只适用于比较简单的判断，可以更加简洁）

语法  var 结果 = 条件 ？ A ：B；（如果条件满足，就是A 否则B）

#### switch-case（根据变量的具体，进行判断）
```javascript
switch（变量）{
  case 值1:
	  语句1;
...
### 中级版（优化）：一趟可以比出一个最大值，每趟下来都可以少比一次
		内循环-j

### 高级版（优化）：假设成立法，如果已经排好序了，后面就不用再排了
1. 在每趟排列前，先假设，已经排好了
...
#### 函数的参数（在函数中，需要发生变化的值，可以定义成函数的参数）
...
## 对象（如果需要描述现实生活对象的特征和行为，就需要js中的对象）
### 概念：无序的键值对的集合
...
### 批量创建对象（将来网站用户，不可能只有一个，就需要批量创建对象）
设计模式（工厂模式、单例模式、观察者模式）
在对象的方法中，this指代当前对象（对象名）
#### 工厂函数（可以将创建对象的代码封装在一个函数中，这个函数就叫工厂函数）
...
### 内置对象（js语法中，内置的一些对象，提供了很多的属性和方法，可以直接用）

#### Math对象（提供了一系列和数学相关的属性和方法）
1. PI Math.PI
2. 求最大最小值 max min
...
## 基本包装类型（解释了简单数据类型，为什么可以直接使用属性和方法）
...
## 面向过程：注重点是过程和步骤，亲力亲为，事无巨细的！！
## 面向对象：关注的是找一个对象，去做某件事
...
### 补充（原先的）
#### 属性搜索原则：自己有就访问自己的，自己没有，去原型链中就近查找
1. 如果自己有这个属性，就访问自己的
...
## 继承（拿来主义，自己没有的属性和方法，别人有，拿过来用，就实现了继承）⭐️⭐️⭐️
### 原型链继承
```javascript
function Person() {
...
## eval(str)可以执行字符串，非常强大，但是不用（风险性太大，会造成全局变量的污染）
```javascript
var a = 100;
...
### 上下文调用模式：（this在这边是可以自定义的，想要this指向谁，函数执行时this就指向谁）
...
### 斐波那契数列（兔子数列）：从第三个数开始，后面的数等于前两个数的和
```javascript
// 直接这么写会有性能问题
function getFib(n) {
  if (n === 1 || n === 2) {
